...
const MODEL_VERSIONS = {
    ...
    'replicate-deepseek-r1': {
        version: '656b5ca19031ecadf6de648f81c7af84cba66eb7a1328fa863d0282a6fa7da7b',
        streaming: true,
        inputParam: 'prompt',
        owner: 'lucataco/deepseek-r1-70b'
    }
};
...
const replicateService = {
    ...
    async generateText(prompt, callback) {
        ...
        try {
            const response = await this.makePredictionRequest(requestData);
            
            if (modelConfig.streaming && callback) {
                // For streaming models, handle the stream directly from the prediction response
                const streamUrl = response.data.urls?.stream;
                if (!streamUrl) {
                    throw new Error('No stream URL provided in prediction response');
                }
                
                return this.handleStreaming(streamUrl, callback);
            } else {
                // For non-streaming models, wait for completion and return full result
                const prediction = await this.waitForModelReady(response.data.id);
